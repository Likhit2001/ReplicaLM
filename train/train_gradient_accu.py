...
print(f"Total Grad Accu step : {grad_accum_steps}")

# Input data Loader
training_data_loader = DataLoader(B,T)
...
for step in range(max_steps):
    ...
    t1 = time.time()
    dt = t1-t0
    tokens_processed = training_data_loader.B * training_data_loader.T * grad_accum_steps
    tokens_per_sec = tokens_processed / dt
    
    print(f"step  {step}  - loss: {loss_acu.item():.6f} - norm: {norm:.4f} - lr: {lr:.4f} - dt: {dt*1000:.2f}ms - tok/sec: {tokens_per_sec}")
